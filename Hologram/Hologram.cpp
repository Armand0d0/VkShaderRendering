...
struct ShaderParamBlock {
   ...
   // float light_color[4];
     ...
     alignas(16) float powers[16];
     alignas(16) float coeff[6];
    //float model[4 * 4];
...
void Hologram::create_buffers() {
    ...
    if (aligned_object_data_size % alignment) aligned_object_data_size += alignment - (aligned_object_data_size % alignment);
        //shell_->print("i = ", (int)(aligned_object_data_size));
...
void Hologram::update_camera() {/*
    ...
    const glm::mat4 view = glm::lookAt(camera_.eye_pos, center, up);
      //   shell_->print("pos : ",camera_.eye_pos);
    float aspect = static_cast<float>(extent_.width) / static_cast<float>(extent_.height);
    const glm::mat4 projection = glm::perspective(100.0f, aspect, 0.001f, 100.0f);
...
void Hologram::draw_object(const Simulation::Object &obj, FrameData &data, VkCommandBuffer cmd) const {
     glm::vec2 resolution = glm::vec2(static_cast<float>(extent_.width) , static_cast<float>(extent_.height));
    if (use_push_constants_) {
        ShaderParamBlock params;
          
        memcpy(params.resolution, glm::value_ptr(resolution), sizeof(resolution));
       ...
       // params.alpha =  obj.alpha ;
        params.time = shell_->time_;   //shell_->print("time : ",params.time);
        /*float *(Powers[5]);
         float *(Coeff[5]);
        for(int i =0; i<5;i++){
           Powers[i] = glm::value_ptr(obj.powers[i]);
           Coeff[i] = glm::value_ptr(obj.coeff[i]);
        }*/
        memcpy(params.pos, (obj.Density.pos),  sizeof(obj.Density.pos));
          memcpy(params.powers, (obj.Density.powers),  sizeof(obj.Density.powers));
            memcpy(params.coeff, (obj.Density.coeff),  sizeof(obj.Density.coeff));

            //sshell_->print("pow[0]: ",params.powers[2]);

        vk::CmdPushConstants(cmd, pipeline_layout_, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(params), &params);
