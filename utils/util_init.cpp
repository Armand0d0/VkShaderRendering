...
//        cout<<"instance_extensions : "<< layer_props.instance_extensions[2].extensionName<<endl;
...
VkResult init_device_extension_properties(struct sample_info &info, layer_properties &layer_props) {
    ...
    do {
        res = vkEnumerateDeviceExtensionProperties(info.gpus[GPU_ID], layer_name, &device_extension_count, NULL);
        if (res) return res;
        ...
        res = vkEnumerateDeviceExtensionProperties(info.gpus[GPU_ID], layer_name, &device_extension_count, device_extensions);
    } while (res == VK_INCOMPLETE);
...
VkResult init_device(struct sample_info &info) {
    ...
    res = vkCreateDevice(info.gpus[GPU_ID], &device_info, NULL, &info.device);
    assert(res == VK_SUCCESS);
...
VkResult init_enumerate_device(struct sample_info &info, uint32_t gpu_count) {
    ...
    vkGetPhysicalDeviceQueueFamilyProperties(info.gpus[GPU_ID], &info.queue_family_count, NULL);
    assert(info.queue_family_count >= 1);

    info.queue_props.resize(info.queue_family_count);
    vkGetPhysicalDeviceQueueFamilyProperties(info.gpus[GPU_ID], &info.queue_family_count, info.queue_props.data());
    assert(info.queue_family_count >= 1);

    /* This is as good a place as any to do this */
    vkGetPhysicalDeviceMemoryProperties(info.gpus[GPU_ID], &info.memory_properties);
    vkGetPhysicalDeviceProperties(info.gpus[GPU_ID], &info.gpu_props);
...
void init_depth_buffer(struct sample_info &info) {
    ...
    vkGetPhysicalDeviceFormatProperties(info.gpus[GPU_ID], VK_FORMAT_D24_UNORM_S8_UINT, &props);
    ...
    vkGetPhysicalDeviceFormatProperties(info.gpus[GPU_ID], depth_format, &props);
    if (props.linearTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
...
// Construct the surface description:
    ...
    for (uint32_t i = 0; i < info.queue_family_count; i++) {
        vkGetPhysicalDeviceSurfaceSupportKHR(info.gpus[GPU_ID], i, info.surface, &pSupportsPresent[i]);
    }
    ...
    res = vkGetPhysicalDeviceSurfaceFormatsKHR(info.gpus[GPU_ID], info.surface, &formatCount, NULL);
    assert(res == VK_SUCCESS);
    VkSurfaceFormatKHR *surfFormats = (VkSurfaceFormatKHR *)malloc(formatCount * sizeof(VkSurfaceFormatKHR));
    res = vkGetPhysicalDeviceSurfaceFormatsKHR(info.gpus[GPU_ID], info.surface, &formatCount, surfFormats);
    assert(res == VK_SUCCESS);
...
void init_swap_chain(struct sample_info &info, VkImageUsageFlags usageFlags) {
    ...
    res = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(info.gpus[GPU_ID], info.surface, &surfCapabilities);
    assert(res == VK_SUCCESS);

    uint32_t presentModeCount;
    res = vkGetPhysicalDeviceSurfacePresentModesKHR(info.gpus[GPU_ID], info.surface, &presentModeCount, NULL);
    assert(res == VK_SUCCESS);
    VkPresentModeKHR *presentModes = (VkPresentModeKHR *)malloc(presentModeCount * sizeof(VkPresentModeKHR));
    assert(presentModes);
    res = vkGetPhysicalDeviceSurfacePresentModesKHR(info.gpus[GPU_ID], info.surface, &presentModeCount, presentModes);
    assert(res == VK_SUCCESS);
...
void init_image(struct sample_info &info, texture_object &texObj, const char *textureName, VkImageUsageFlags extraUsages,
    ...
    vkGetPhysicalDeviceFormatProperties(info.gpus[GPU_ID], VK_FORMAT_R8G8B8A8_UNORM, &formatProps);
